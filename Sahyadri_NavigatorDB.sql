...
CREATE TABLE `trekk_details` (
  ...
  `COMBO_PACK` varchar(45) NOT NULL,
...
CREATE TABLE `trekker_reg_event` (
  ...
  CONSTRAINT `TREKK_ID` FOREIGN KEY (`EVENT_ID`) REFERENCES `trekk_details` (`TREKK_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `USER_ID` FOREIGN KEY (`USER_ID`) REFERENCES `registration` (`USER_ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `trekking_club_trekk_reg` (
  ...
  CONSTRAINT `F_USER_ID` FOREIGN KEY (`USER_ID`) REFERENCES `registration` (`USER_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Dump completed on 2024-02-07 12:52:52
